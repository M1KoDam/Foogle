...
class Indexer:
    ...
    def find(self, input_phrase: str, index=None, files_word_count=None) -> list:
        if index is None or files_word_count is None:
            index, files_word_count = self.build_index()
        and_flag, or_flag, not_flag, phrases = find_bool_operation(input_phrase.lower())

        results = []
        for phrase in phrases:
            processed_phrase = self.process_phrase(phrase)
            results.append(self.find_phrase(index, processed_phrase, files_word_count))
    ...
    def match_phrase_to_indexer(self, index: dict, processed_phrase: list):
        files_to_index = {}
        new_phrase = []

        coll = list(index.keys())
        for word in processed_phrase:
            ...
            for ld_word in ld_words:
                if best_ld_word in files_to_index.keys():
                    for key, value in index[ld_word].items():
                    ...
                    files_to_index[best_ld_word] = index[ld_word].copy()
    ...
    def find_phrase(self,
        ...
        files_words, processed_phrase = self.match_phrase_to_indexer(index, processed_phrase)
        ...
        for file_name, words in files_words.items():
            if len(processed_phrase) == 1:
                power = tf_idf(word_in_file=len(words[processed_phrase[0]]),
                               ...
                               files_count_with_word=len(index[processed_phrase[0]].keys()))

            else:
                temp_indexes = []
                for i, value in enumerate(words.values()):
                    temp_indexes.extend([ind - i for ind in value])
    ...
    def build_index(self) -> (dict, dict):
